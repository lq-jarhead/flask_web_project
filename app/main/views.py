...
#自己添加删除post方法，问题是在index和在个人profile删除返回都是index.html怎么实现，删除后直接刷新呢？
#删除之前应该有确认按钮，后期添加吧
...
@login_required
def edit(id):
    post = Post.query.get_or_404(id)#get_or_404()返回指定主键对应的行，若没有找到指定的主键，则终止请求，返回404错误响应
...
def index():
    ...
    if show_followed:
        query=current_user.followed_posts #返回的是Post对象，有属性的对象，而不是值
    else:
        query = Post.query
...
def edit_profile():
    ...
    #下面的current_user怎么去理解？见many_linking.md
...
def edit_profile_admin(id):
    ...
    #亲测下面的user.email改为curren_user也可行。10/31
...
def follow(username):
    ...
    if user is None:
        flash('Invalid user.')
        return redirect(url_for('.index'))
    #个人觉得，是follow跳转过来的，下面这个if语句没必要啊
...
#有user.html中的followers按钮跳转过来，显示关注用户的人 followers of lengqian
...
#由user.html中的followed按钮跳转过来，显示用户关注的人 followed by lengqian
